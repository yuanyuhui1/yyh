...
//--------------------------------------------------------------------------------------------------
/*!
 \brief       Calculate the energy flow for steam or selection simulation energy flow.
 \author      zuochen wang
 \date        2012-07-02
 \param    
	inMF - input mass flow
 \return    
    energy flow 
\warning     none
 \test
...
*/
//--------------------------------------------------------------------------------------------------
TFLOAT CalculateSteamQp(TFLOAT inMF)
{
	...
	{  
	  	baseUnitValue = (TFLOAT)( inMF *  fabs((TDOUBLE)(mVMeasurementDynamicDuplicated.Hw - mVMeasurementDynamicDuplicated.Hc)));
		lSimState = eFALSE;
	}

	//LowCutoff and damping
	lCutoffAlarm = CalculateQ(CDT_STEAM_POWER, &baseUnitValue, &lDampValue, lSimState, lRangHiLim, lCutoffPer, lDampTime);
	if (IsPVMatchWithQ(CDT_STEAM_POWER))
	{

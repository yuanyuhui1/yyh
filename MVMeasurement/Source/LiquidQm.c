...
*/
//--------------------------------------------------------------------------------------------------
TFLOAT CalculateLiquidQm(TFLOAT inVF,TFLOAT inDensityReal,TFLOAT inTReal)
{
	...
	lCutoffAlarm = CalculateQ(CDT_LIQUID_MASS, &baseUnitValue, &lDampValue, lSimState, lRangHiLim, lCutoffPer, lDampTime);
	if (IsPVMatchWithQ(CDT_LIQUID_MASS))
	{
